...
export function Apple(props: { scale?: number } & SVGProps) {
  const { scale = 0 } = props

  const width = scale <= 0 ? undefined : scale * WIDTH
  const height = scale <= 0 ? undefined : scale * HEIGHT

  return (
    <Svg {...props} {...{ width, height }} viewBox="0 0 496.255 608.728">
      <Path
